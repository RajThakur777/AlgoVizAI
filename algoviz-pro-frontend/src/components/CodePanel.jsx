...
export const ALGO_CODE_DATABASE = {
    ...
    'dijkstra': [
        ...
        { number: 5, text: '    let {node, d} = pq.dequeue_min();' }, // Dequeue/Extract Min
        { number: 6, text: '    for (let edge of adj[node]) {' },
        { number: 7, text: '      let cost = d + edge.weight;' },
        { number: 8, text: '      if (cost < dist[edge.to]) {' },
        { number: 9, text: '        pq.update_priority(edge.to, cost);' }, // Relax and update
        { number: 10, text: '      }' },
        { number: 11, text: '    }' },
        { number: 12, text: '  }' },
        { number: 13, text: '}' }
    ...
    return (
        <div className="flex flex-col h-full bg-slate-900/50 border border-indigo-500/20 shadow-2xl rounded-2xl overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-black/30 font-mono text-sm custom-scrollbar">
                <pre>
                    {code.map((line) => (
                        <div key={line.number} className={`py-1.5 transition-colors duration-200 ${line.number === highlightedLine ? 'bg-indigo-600/40 border-l-4 border-indigo-500 pl-2 text-white' : 'text-gray-400 pl-3'}`}>
